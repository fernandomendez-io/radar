...
export default function TacticalMapPage() {
  ...
  const flightsRef = useRef<Flight[]>([]); // <-- ref so nudge can read latest without dep
    ...
    const fetchData = async () => {
      try {
        ...
        if (mRes.ok) {
          ...
          if (mData?.length > 0) {
            ...
            setMetar({
              raw: kdfw.rawOb,
              category: category as MetarData["category"],
      ...
      try {
        ...
        if (!res.ok) {
          ...
          console.error("Flights API error:", res.status, err);
          return;
        }
        ...
        // Seed trail with current position on fetch
        data.forEach((f) => {
          if (!f.on_ground && f.velocity >= 10) {
            if (!trails.current[f.icao24]) {
              trails.current[f.icao24] = [];
            }
  ...
  // No deps — uses refs only so the interval never resets
  useEffect(() => {
    const updateTrails = () => {
      ...
      source.setData({
        ...
        features: Object.entries(trails.current)
          ...
          .map(([icao24, coords]) => {
            ...
            const isCompany =
              flight?.callsign.startsWith("ENY") ||
              flight?.callsign.startsWith("AAL");
            return {
    ...
    const nudge = setInterval(() => {
      ...
      // Sync state for HUD (targets count etc) every nudge
  ...
  useEffect(() => {
    ...
    flights.forEach((f) => {
      ...
      if (markers.current[f.icao24]) {
        ...
        if (popup?.isOpen()) popup.setHTML(popupHTML);
